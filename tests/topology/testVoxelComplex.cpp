...
struct Fixture_complex_diamond {
  ...
  FixtureObject& create_object() {
    ...
    FixtureDigitalTopology topo(adjF, adjB,
...
TEST_CASE_METHOD(Fixture_complex_diamond, "insertVoxel", "[insert][close]" ){
  ...
  auto & ks = vc.space();
  auto s3 = vc.nbCells(3); auto s2 = vc.nbCells(2);
  auto s1 = vc.nbCells(1); auto s0 = vc.nbCells(0);
  SECTION( "insertVoxel in border" ){
    Point p{0,0,4};
    auto vc_new = vc;
    vc_new.insertVoxelPoint(p);
    auto sa3 = vc_new.nbCells(3); auto sa2 = vc_new.nbCells(2);
    auto sa1 = vc_new.nbCells(1); auto sa0 = vc_new.nbCells(0);
    auto d3 = sa3 - s3; CHECK(d3 == 1);
    auto d2 = sa2 - s2; CHECK(d2 == 5);
    auto d1 = sa1 - s1; CHECK(d1 == 8);
    auto d0 = sa0 - s0; CHECK(d0 == 4);
  }
  SECTION( "insertVoxel isolated" ){
    ...
    auto sa1 = vc_new.nbCells(1); auto sa0 = vc_new.nbCells(0);
    auto d3 = sa3 - s3; CHECK(d3 == 1);
    auto d2 = sa2 - s2; CHECK(d2 == 6);
    auto d1 = sa1 - s1; CHECK(d1 == 12);
    auto d0 = sa0 - s0; CHECK(d0 == 8);
  }
  SECTION( "insertVoxel interior" ){
    ...
    auto sa3 = vc_new.nbCells(3); auto sa2 = vc_new.nbCells(2);
    auto sa1 = vc_new.nbCells(1); auto sa0 = vc_new.nbCells(0);
    auto d3 = sa3 - s3; CHECK(d3 == 0);
    auto d2 = sa2 - s2; CHECK(d2 == 0);
    auto d1 = sa1 - s1; CHECK(d1 == 0);
    auto d0 = sa0 - s0; CHECK(d0 == 0);
  }
}
...
TEST_CASE_METHOD(Fixture_complex_diamond, "Neighbors from Object and KSpace",
  ...
  SECTION( " comparing neighbors from Kspace and from Object" ){ {
    ...
    auto point_from_kspace_1 = cell.myCoordinates;
    SECTION(" points from Kspace and digital set have different coords "){
      // point coordinates are different in the Kspace and in DigitalSet.
      CHECK(point_from_objectSet_1 != point_from_kspace_1);
      SECTION("But uCoords from KSpace gives same coords than the object set" ){
    ...
    SECTION( "properNeighborhood from Object with full topology"
      ...
      SECTION( "properNeighborhood from KSpace"
        ...
        auto neigh_k =
          vc.space().uNeighborhood(cell);
        CHECK( neigh_k.size() != neigh_obj.size() );

        auto propN_k =
          vc.space().uProperNeighborhood(cell);
...
TEST_CASE_METHOD(Fixture_complex_diamond, "Test Object Wrappers", "[object]" ){
  ...
  SECTION( "querying voxel simplicity" ){
    ...
    for( auto it = vc.begin(dim_voxel); it != vc.end(dim_voxel) ; ++it ){
      if(vc.isSimple(it->first)) ++nsimples;
      else non_simple_cells.emplace_back(it->first);
    }
...
TEST_CASE_METHOD(Fixture_complex_diamond, "connectedComponents", "[object][functions][components]" ){
  auto & vc = complex_fixture;
...
TEST_CASE_METHOD(Fixture_complex_diamond, "Cliques Masks K_2", "[clique]" ){
  ...
  for( auto && n : neigh6.pointSet() ) {
    cliques_p.emplace_back(
        vc.K_2(p_cell, n, verbose)
        );
...
TEST_CASE_METHOD(Fixture_complex_diamond, "Cliques Masks K_1", "[clique]" ){
  ...
  SECTION( "K_1 mask from a linel" ){
    auto linel = vc.space().uCell(cell.myCoordinates + Point{0, 1, 1});
...
struct Fixture_complex_fig4 {
  ...
  FixtureObject& create_object() {
    ...
    Point a1( 0, 0, 0 ); fig4_set.insertNew(a1);
    Point a2( 0, -1, 0 ); fig4_set.insertNew(a2);
    Point a3( 1, -1, 0 ); fig4_set.insertNew(a3);
    Point c1( 0, 1, -1 ); fig4_set.insertNew(c1);
    Point c2( 1, 1, -1 ); fig4_set.insertNew(c2);
    Point c3( 1, 2, -1 ); fig4_set.insertNew(c3);
    Point c4( 0, 2, -1 ); fig4_set.insertNew(c4);
    Point b1( 0, 2, 0); fig4_set.insertNew(b1);
    Point b2( -1, 3, 0); fig4_set.insertNew(b2);
    Point l1( 1, 0, -2 ); fig4_set.insertNew(l1);
    Point l2( 2, 0, -2 ); fig4_set.insertNew(l2);
    Point r1( 1, 3, -2 ); fig4_set.insertNew(r1);
...
struct Fixture_isthmus {
  ...
  FixtureObject& create_object() {
    ...
    Point c00( 0, 0, 0 ); fig6_set.insertNew(c00);
    Point c01( -1, 0, 0 ); fig6_set.insertNew(c01);
    Point c02( -2, 0, 0 ); fig6_set.insertNew(c02);
    Point c03( -2, 0, 1 ); fig6_set.insertNew(c03);

    Point c10( 0, 1, 0 ); fig6_set.insertNew(c10);
    Point y( -1, 1, 0 ); fig6_set.insertNew(y);
    Point c12( -2, 1, 0 ); fig6_set.insertNew(c12);
    Point c13( -2, 1, 1 ); fig6_set.insertNew(c13);

    Point c20( 0, 2, 0 ); fig6_set.insertNew(c20);
    Point c21( -1, 2, 0 ); fig6_set.insertNew(c21);
    Point c22( -1, 2, 1 ); fig6_set.insertNew(c22);
    Point c33( -1, 2, -1 ); fig6_set.insertNew(c33);

    Point c30( -1, 3, 0 ); fig6_set.insertNew(c30);
    Point c31( -1, 3, 1 ); fig6_set.insertNew(c31);

    Point x( -1, 4, 0 ); fig6_set.insertNew(x);

    Point c50( -1, 5, 0 ); fig6_set.insertNew(c50);
    Point c51( -1, 5, -1 ); fig6_set.insertNew(c51);
    Point c52( -2, 5, 0 ); fig6_set.insertNew(c52);

    Point c60( -1, 6, 0 ); fig6_set.insertNew(c60);
    Point c61( -2, 6, 0 ); fig6_set.insertNew(c61);
    Point c62( -2, 6, -1 ); fig6_set.insertNew(c62);
...
struct Fixture_X {
  ...
  FixtureObject& create_object() {
    ...
    Point c00( 0, 0, 0 ); center_set.push_back(c00);
    Point c01x( -1, 0, 0 ); center_set.push_back(c01x);
    Point c10x( 1, 0, 0 ); center_set.push_back(c10x);
    Point c02x( -2, 0, 0 ); center_set.push_back(c02x);
    Point c20x( 2, 0, 0 ); center_set.push_back(c20x);

    Point c01y( 0, -1, 0 ); center_set.push_back(c01y);
    Point c10y( 0, 1, 0 ); center_set.push_back(c10y);
    Point c02y( 0, -2, 0 ); center_set.push_back(c02y);
    Point c20y( 0, 2, 0 ); center_set.push_back(c20y);
...
TEST_CASE_METHOD(Fixture_X, "X Thin", "[x][persistence][isthmus][thin][function]"){
  ...
  SECTION( "persistence value of 1 is equivalent to the assymetric algorithm" ){
    ...
    // 10% tolerance
    CHECK(vc_assymetric.nbCells(3) == Approx(vc_new.nbCells(3)).epsilon(0.1) );
  }
  SECTION( "persistence value: infinite is equivalent to ultimate skeleton" ){
    auto vc_new = persistenceAsymetricThinningScheme< FixtureComplex >(
        vc, selectRandom<FixtureComplex>, skelEnd<FixtureComplex>, 100, verbose);
...
TEST_CASE_METHOD(Fixture_X, "X Thin with Isthmus, and tables", "[x][isthmus][thin][function][table]"){
  ...
  SECTION( "Compute with skelIsthmus" ){
    ...
    auto vc_new = asymetricThinningScheme< FixtureComplex >(
        vc, selectRandom<FixtureComplex>, skelIsthmus<FixtureComplex>, verbose);
    trace.endBlock();
  }
...
TEST_CASE_METHOD(Fixture_X, "X DistanceMap", "[x][distance][thin]"){
  ...
  SECTION( "Distance Map" ){
    ...
    auto selectDistMax =
      [&dt](const FixtureComplex::Clique & clique){
