...
bool testSurfelAdjacency()
{
  ...
  // JOL: Disabled, since Board is not adapted to CodedKhalimskySpaceND.
  // if ( K.dimension == 2 )
  //   {
  //     Board2D board;
...
testKSpaceElementaryMethods()
{
  ...
  std::cout << "Dp0     dirs =";
  K.printSCell( std::cout, Dp0 );
  for ( DirIterator q = K.sDirs( Dp0 ); q != 0; ++q ) std::cout << " " << *q;
  std::cout << std::endl;
  std::cout << "Dp1     dirs =";
  K.printSCell( std::cout, Dp1 );
  for ( DirIterator q = K.sDirs( Dp1 ); q != 0; ++q ) std::cout << " " << *q;
  std::cout << std::endl;
  std::cout << "Dp0_Dp1 dirs =";
  K.printSCell( std::cout, Dp0_Dp1 );
  for ( DirIterator q = K.sDirs( Dp0_Dp1 ); q != 0; ++q ) std::cout << " " << *q;
  std::cout << std::endl;
  return ok;
}
///////////////////////////////////////////////////////////////////////////////
